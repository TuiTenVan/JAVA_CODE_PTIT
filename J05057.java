...
class BangDiemThiSinh{
    private String ma, name;
    private double toan, ly, hoa;
    public BangDiemThiSinh(String ma, String name, double toan, double ly, double hoa){
        this.ma = ma;
        this.name = name;
        this.toan = toan;
        this.ly = ly;
    ...
    public String toString(){
        return ma + " " + name + " " + getUutien() + " " + getKhongUuTien() + " ";
    }
}
public class J05057 {
    public static void main(String[] args){
        ...
        int t = sc.nextInt();
        sc.nextLine();
        List<BangDiemThiSinh> list = new ArrayList<>();
        while(t-- > 0) {
            ...
            BangDiemThiSinh ts = new BangDiemThiSinh(ma, name, toan, ly, hoa);
            ts.chuanHoa();
            list.add(ts);
        }
        // list.sort((s1, s2) -> Double.compare(s2.tong(), s1.tong()));
        for(BangDiemThiSinh i : list){
            if(i.tong() >= 24){
